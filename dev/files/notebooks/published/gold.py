...
# MAGIC   not config. There are no calculated/derived columns -- every Gold
# MAGIC   column is read directly from Silver by the same name.
...
def process_gold_entity(spark, catalog: str, entity: str, schemas: Dict) -> Dict:
    ...
    # All Gold columns are read directly from Silver by name (no more
    # calculated/derived columns) -- the audit column is generated by the
    # framework, not read from Silver, so it's excluded here.
    ...
    # mismatches early instead of inside a cryptic MERGE error.
...
def _bootstrap_gold_table(
    ...
    if silver_count > 0:
        ...
        # Every Gold column is read directly from Silver by name, plus the
        # framework-generated audit columns stamped with one atomic timestamp.
        col_exprs = [F.col(name).alias(name) for name, _ in gold_columns]
...
def _incremental_gold_merge(
    ...
    # Every Gold column is read directly from Silver by name, plus the
    # framework-generated audit columns stamped with one atomic timestamp.
    col_exprs_merge = [F.col(name).alias(name) for name, _ in gold_columns]
